...
let alarm_m = null;
let timeoutId = null;
let menu_or_stage = false; //全局模式。 Stage statu.
//~ let menu_or_stage = true;	//菜单模式。 Menu statu.
...
const Indicator = GObject.registerClass(
	class Indicator extends PanelMenu.Button {
		_init() {
			...
			this.item.actor.add_child(this.da);
			this.item.reactive = false;
			this.width = 50;
			this.mode();
			this.connect("button-press-event", this.own_click.bind(this));
		}

		mode() {
			this.background_color = Clutter.Color.from_string(menu_or_stage ? "gray" : "green")[1];

			if (menu_or_stage) {
				global.stage.remove_child(this.item);

				this.menu.addMenuItem(this.item);
				this.item.background_color = Clutter.Color.from_string("dark gray")[1];
				//~ const c = this.item.get_background_color(); log(c.red+","+c.green+","+c.blue+":"+c.alpha);

			} else {
				this.menu.box.remove_all_children();
				this.menu.close();
				this.item.background_color = Clutter.Color.new(0, 0, 0, 0);

				global.stage.add_child(this.item); //加到全局，才透明。
				this.item.visible = false;
				//~ this.item.opacity = 200;
			}
		};

		own_click(actor, event) {
			const altkey = event.get_state() & Clutter.ModifierType.MOD1_MASK;
			if (altkey) {
				menu_or_stage = !menu_or_stage;
				this.mode();
			} else {
				if (menu_or_stage) {
				} else {
					const [x, y] = global.get_pointer();
					this.item.set_position(x - size / 2, y + 50);
		...
		destroy() {
			if (!menu_or_stage) global.stage.remove_child(this.item);
			super.destroy();
		}
	});
...
class Extension {
	...
	enable() {
		timeoutId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 10, () => {
			if (this._indicator.menu.isOpen || !this._indicator.menu_or_stage) {
